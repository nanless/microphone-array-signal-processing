...
The tutorial provides key derivations, reproducible numerical examples, validity limits, 34 script-generated figures, and NumPy/standard-library teaching code mapped to the equations. It adds 36 executable exercises and 23 synthetic audio files in 6 groups; see [exercises and audio experiments](codes/research/05_exercises_and_audio.md) for inputs, answers and listening conditions. These files are not natural speech or formal listening-test data.
...
| `figures/` | 34 figures (`fig01`–`fig34_*.png`), all generated by scripts and reproducible |
...
| `codes/audio/` | 23 WAV files in 6 groups synthesized for this book, with parameters and hashes; generated by script, not edited directly |
...
# 3. Generate 23 audio files first, then 34 figures (Figure 34 reads the generated audio)
...
- **Path A (from scratch, 2–3 weeks)**: Guide → 01 → 11.1/11.3 → 02/03 → 04 (GCC+SRP) → 05 (DSB+MVDR) → 06/07/08 → 09 → run `codes/examples/` and reproduce all 34 figures.

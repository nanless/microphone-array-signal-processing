...
| `figures/` | 38 figures (`fig01`–`fig38_*.png`), all generated by scripts and reproducible |
...
| `codes/audio/` | 55 WAV files in 13 groups synthesized for this book, with parameters and hashes; generated by script, not edited directly |
...
| `dist/` | Combined PDF (`microphone-array-tutorial.pdf`) and HTML; the PDF has 125 bookmarks: 14 top-level, 86 second-level and 25 third-level |
...
# 3. Generate 55 audio files first, then 38 figures (Figures 34–36 read the generated audio)
...
- **Path A (from scratch, 2–3 weeks)**: Guide → 01 → 11.1/11.3 → 02/03 → 04 (GCC+SRP) → 05 (DSB+MVDR) → 06/07/08 → 09 → run `codes/examples/` and reproduce all 38 figures.

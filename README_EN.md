...
The tutorial provides key derivations, reproducible numerical examples, validity limits, 35 script-generated figures, and NumPy/standard-library teaching code mapped to the equations. It adds 61 executable exercises and 36 synthetic audio files in 9 groups; see [exercises and audio experiments](codes/research/05_exercises_and_audio.md) for inputs, answers and listening conditions. These 36 files are not natural speech or formal listening-test data. A separate set of 4 WAV files contains a real 16-channel DEMAND river-noise excerpt and three derivatives under CC BY-SA 3.0, for noise-power and interchannel-correlation experiments.
...
| `figures/` | 35 figures (`fig01`–`fig35_*.png`), all generated by scripts and reproducible |
...
| `codes/audio/` | 36 WAV files in 9 groups synthesized for this book, with parameters and hashes; generated by script, not edited directly |
...
| `dist/` | Combined PDF (`microphone-array-tutorial.pdf`) and HTML; the PDF has 120 bookmarks: 14 top-level, 85 second-level and 21 third-level |
...
# 3. Generate 36 audio files first, then 35 figures (Figures 34 and 35 read the generated audio)
...
- **Path A (from scratch, 2–3 weeks)**: Guide → 01 → 11.1/11.3 → 02/03 → 04 (GCC+SRP) → 05 (DSB+MVDR) → 06/07/08 → 09 → run `codes/examples/` and reproduce all 35 figures.

...
| 06 | `06_aec.md` | 图18～20、图26～32、图36，共 11 张，另引用图23 | 较难 | 60～80 分钟，建议分两次读 |
...
NLMS（归一化最小均方自适应滤波）给出基础更新式。PBFDAF（分区块频域自适应滤波）把长路径分区，按频点使用多个历史参考块更新；在第 6 章规定的同一分块、误差和归一化口径下，它就是频域多抽头 NLMS 的具体实现，不是与之并列的新算法。子带方法、IPNLMS 和 FDKF（频域卡尔曼滤波）改变信号表示、系数分配或状态估计。扬声器非线性会限制线性 AEC，需根据失真来源增加非线性模型或神经网络。DTD（双讲检测）用于控制近端和远端同时活动时的自适应更新。
...
6. 终点：重新生成并解释 36 张图，再在可用的多通道硬件上实现一个可测方向响应的基线系统。
...
另有 72 道带答案的[章节代码练习与音频实验](../codes/research/05_exercises_and_audio.md)，从第 1 章到附录 B 使用 E01-01 等稳定编号。第 6 章 E06-07～E06-10 是无音频的算法边界手算。每道题对应输入、执行代码和可核查结果，不替代原有练习题号。
...
- [ ] 对照第 8 节画一遍 36 张图，打开 `figures/` 对照正文看图。

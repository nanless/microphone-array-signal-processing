...
全套教程配了 41 张图，图 18～20、图 26～32 和图 36～39 在第 6 章。插图分为几何示意、指定条件下的仿真和系统框图，阅读时要同时核对图注中的参数。
...
文里配了 41 张图。第一次阅读时，可以先看图和算例，再回到公式核对条件与计算步骤。
...
| 编号 | 文件 | 配图 | 难度 | 建议学习单元 |
|---|---|---|---|---|
| 00 | `00_overview.md`（本页） | 图34 | 入门 | 导航与复现入口 |
| 01 | `01_problem-definition.md` | 图1 | 入门 | 问题与评价口径 |
| 02 | `02_basics-signal-model.md` | 图2～图7，共 6 张 | 进阶 | 时延与流形；STFT与统计；波束度量，分三单元 |
| 03 | `03_array-geometry.md` | 图8～图10，共 3 张 | 进阶 | 几何可观测性；标定与稀疏阵 |
| 04 | `04_doa-estimation.md` | 图11～图14、图33，共 5 张 | 较难 | 时差与扫描；子空间；宽带与分辨率实验 |
| 05 | `05_beamforming.md` | 图15～图17，共 3 张 | 较难 | 约束解；稳健设计；掩码与工程接口 |
| 06 | `06_aec.md` | 图18～20、图26～32、图36～39，共 14 张，另引用图23 | 较难 | 参考与卷积；自适应更新；控制与评测；工业接口，分四单元 |
| 07 | `07_wpe-dereverberation.md` | 图21、图24，共 2 张 | 进阶 | 预测模型；求解与时间边界 |
| 08 | `08_speech-separation.md` | 0 张专属图，引用图23 | 进阶 | 盲分离；掩码/活动；深度方法与输出评测 |
| 09 | `09_source-tracking.md` | 本章专属图22，另引用图23 | 进阶 | 状态递推；数据关联与时间边界 |
| 10 | `10_engineering-practice.md` | 图25、图40，另引用图23 | 进阶 | 模块依赖；设备时序；资源与评测，分三单元 |
| 11 | `11_selection-guide.md` | 0 张专属图 | 入门 | 需求约束；候选比较；验收设计 |
| 附录 A（文件 12） | `12_appendix-symbols-math.md` | 0 张专属图 | 查阅 | 按符号、数学工具和反例查阅 |
| 附录 B（文件 13） | `13_appendix-guide.md` | 图35、图41 | 查阅 | 按学习路径、排错、综合练习查阅 |

每个单元都包括概念、推导和相应练习。完成判据是能说明变量与假设、独立复算一个例子、解释失败边界，再运行代码核对；阅读速度无法代表推导和实验需要的时间。因此不为全部精读给出统一小时数，长章可按表中单元分次完成。
...
[§4.10 四麦分数采样时差](./04_doa-estimation.md#sec-4-10)把 4 cm 阵列的几何时差、理想相量计算和四通道合成 WAV 放在同一练习中；它只验证指定模型，不替代真实房间的定位评测。
...
14 张图（图18～20、图26～32、图36～39，另引用图23），较难。图 18、图 26～32 解释回声模型、收敛与工程条件，图 36 展示非线性残留；图 37～38 分别说明子带交叉项和 IPNLMS/RLS/Kalman 的不同状态，图 39 拆开 PBFDAF 的预测和更新。
...
三条路线包括经典盲源分离（AuxIVA、ILRMA、MNMF、TRINICON）、GSS（引导源分离，用说话人活动标注处理输出排列）和深度分离（掩码估计、深度聚类、PIT、Conv-TasNet、状态空间模型及查询型分离）。§8.2 与图 23 说明处理顺序；§8.3、§8.4 和 §8.6 分别展开三条路线。实际模块启用条件见 §10.1。
...
本章的图 40 展示采样时钟漂移逐渐改变双麦平均的幅度，图 25 展示端到端延迟关键路径，另引用图 23 的参考依赖关系。图 23 不表示每个项目都要启用全部模块。第 10 章按启用条件说明 AEC、WPE、定位、波束、NS 和后端的取点关系，并把延迟分为前瞻、缓冲、计算、调度和解码关键路径。
...
11.4 用四类场景演示怎样从条件推导候选方案；11.5 把方案写成可验证规格；11.6 配有九道练习，均有对应的可运行代码。

本章把任务和约束转成候选方案，给出 AEC、WPE 和分离的启用条件，并示范怎样把几何、WNG/DI 权衡和资源预算写成可测的验收规格。

### [12 · 附录 A：符号、术语、数学](./12_appendix-symbols-math.md)

查阅用。12.1 节是符号表。12.2 节是术语词典，按信号与数学基础、房间声学、自适应滤波与分离、波束形成与阵列度量、定位与追踪、系统与硬件分组。12.3 节介绍复数、分贝、卷积和相关、期望和协方差、最小二乘与特征分解；12.4 节用五道代码题复算卷积、复二阶矩、秩亏最小二乘，并区分频谱相位加权与空间噪声白化。

遇到未定义符号、术语或数学步骤时，可按主题查阅，无需顺序通读。

### [13 · 附录 B：路径、地图、前沿、排错、练习、复现](./13_appendix-guide.md)

查阅加动手。13.1 节是学习路径。13.2 节是教材、会议、期刊、挑战赛与历史地图。13.3 节介绍八个研究方向与泛化失配。13.4 节是排错指南，13.5 节是公式速查卡，13.6 节有 17 道练习题及共同增益代码题，13.7 节说明怎样复现 41 张图；E13-02 配合图41检查插值失真。
...
2. 02 按三个学习单元精读。推导可以先跳，重点是会读图 2～图 7。
3. 03 读表格与算例。04 读 4.2、4.3 节与算例，对照看图 12、图 14。05 读 5.1、5.2、5.4 节，完成一次手算，对照看图 16。
4. 第 6 章读 §6.1.1、§6.1.2、§6.6（AEC 在阵列里放哪）和 §6.8，对照看图 18。第 7 章读 §7.1 的直觉部分和 §7.8 的保护延迟、抽头数换算，对照看图 21、图 24。第 8 章读 §8.1 的分离任务和 §8.4 的掩码用途。第 9 章读 §9.2 的卡尔曼滤波五步。
5. 阅读 §10.1、§10.4 和 §10.6，了解参考链路、资源口径与房间仿真。按需查附录 A，再做附录 B 的第 2、3、5、7、10、11、14 题，并用 §13.4 检查常见问题。
6. 终点：重新生成并解释 41 张图，再在可用的多通道硬件上实现一个可测方向响应的基线系统。

### 路径 B：工程实现（形成可验证的前端方案）
...
| 图16 | fig16_beampattern | 05 | 指定模型仿真 | 三条曲线保持目标方向单位响应；DSB/MVDR 用 $d=\lambda/2$，超指向用 $d=0.2\lambda$，MVDR 干扰方向为 20°；条件不同，不可横向按曲线高低排名 |
...
| 图39 | fig39_aec_pbfdaf_flow | 06 | 算法流程图 | 播放参考、声学路径、分区频域预测、有效区残差、冻结控制及梯度合法区投影 |
| 图40 | fig40_clock_drift | 10 | 解析与PCM对照 | 两时钟相差100 ppm；同索引时间差、单频平均包络与双音功率；理想共同时钟目标不是已测补偿器 |
| 图41 | fig41_interpolation_error | 13 | 解析曲线与PCM测量 | 固定分数延迟的幅度失真；两次半采样与纯一采样延迟的区别 |
...
再运行第 13.7 节列出的绘图命令，可重新生成 41 张图；运行时间取决于机器。具体构建步骤和错误处理见 `scripts/README.md`。Windows 上需把命令中的 `.venv/bin/python` 换成 `.venv\Scripts\python`。

另有 117 道带答案的[章节代码练习与音频实验](../codes/research/05_exercises_and_audio.md)，从第 1 章到附录 B 使用 E01-01 等稳定编号。第 6 章 E06-07～E06-20 包含算法边界手算与子带、IPNLMS、RLS、Kalman 的小规模复算。每道题对应输入、执行代码和可核查结果；附录 B 的 1～17 题另用一套编号。

68 个 WAV 样本由本书合成，分为 16 组，覆盖对齐平均、四麦分数采样时差、AEC、WPE、已知矩阵解混、工程失真、双声道声像，以及相关噪声、极性错误、病态求逆、非线性回声、四种 AEC 自适应方法、两带跨项、功率谱减、时钟漂移和线性插值失真。先运行 `codes/examples/generate_audio_samples.py`，再按手册比较参考、输入和输出；网页提供不自动播放的试听控件。

另有独立清单管理的 [GSS 五路合成音频与中间状态](../codes/gss_audio/MANIFEST.json)、[自由场移动声源三路音频与轨迹真值](../codes/moving_audio/MANIFEST.json)，以及附录 B 六位置房间题的 18 个白噪声合成 WAV。三套补充资产均不计入主清单 68 个样本。

三组反例见附录 B 图 35，非线性残留见第 6 章图 36。图 37 解释两带子带模型的交叉项；图 38 分别展示 IPNLMS、RLS 和卡尔曼滤波的受控状态变化，三幅子图不构成性能排名。图 39 展示分区块频域自适应滤波（PBFDAF）如何用播放参考的历史块预测回声、计算残差并更新滤波器。

对应的 AEC 合成音频在研究手册第 15～16 节；谱减试听与前奏数字在第 18 节。音频可用于听辨指定合成条件下的差别，图 39 则用于核对 PBFDAF 的信号与更新路径。
...
先按第 8 节运行章节教学基线，确认公式约定和边界输入，再重绘 41 张图。之后可按锁定版本用 pyroomacoustics 建立房间仿真基线。公开数据、模型和硬件实验应根据任务、许可证与设备条件选择，练习入口见附录 B 的 §13.1、§13.6。
...
- [ ] 阅读第 2 章，学会读图 7（主瓣、旁瓣、零陷、主瓣宽度、栅瓣）。
- [ ] 按第 2 节的学习单元读第 4～8 章；每完成一种方法，先手算对应练习，再用代码复核。
- [ ] 对照第 8 节画一遍 41 张图，打开 `figures/` 对照正文看图。

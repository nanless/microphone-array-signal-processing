...
查阅加动手。13.1 节是学习路径。13.2 节是教材、会议、期刊、挑战赛与历史地图。13.3 节介绍九个研究方向与泛化失配。13.4 节是排错指南，13.5 节是公式速查卡，13.6 节有 17 道练习题，13.7 节说明怎样复现 33 张图。
...
空间线：04 找方向，09 跟轨迹，05 指波束。02、05 和 08 介绍的时频掩码可用于估计目标或噪声协方差，再交给 MVDR 或分离模块。
干扰处理线：06 讲回声消除，07 讲去混响，08 讲语音分离；启用条件和参考依赖见 §10.1 与图 23。
工程线：10 讲链路、硬件、评测、芯片，看完去 11 定方案。
附录 A（文件 12）汇总符号、术语和数学。附录 B（文件 13）提供学习路径、领域地图、前沿、排错、练习和复现说明。完成第 11 章后，工程读者可到 §10.10 制定联调与验收计划，遇到问题查 §13.4；研究读者可读 §13.3，再做 §13.6 的第 16、17 题，分别练习房间仿真和论文系统复核。两篇附录均可按需查阅。
...
| 图11 | fig11_doa_spectrum | 04 | 仿真 | 指定高信噪比、充足快拍条件下的三种谱形；不检验分辨极限 |
| 图12 | fig12_gcc_phat | 04 | 示意＋仿真 | 真值约 0.933 格；固定噪声样例经 16 倍时延网格插值得 58.59375 μs，即 0.9375 格、约 30.16° |
| 图13 | fig13_gcc_reverb | 04 | 说明性仿真 | 在 16 kHz、64 ms、无传感器噪声的随机衰减尾模型中，每点 150 次固定种子试验；正确率带 95% Wilson 区间，峰对比度带四分位区间；只比较随 $T_{60}$ 增大的趋势 |

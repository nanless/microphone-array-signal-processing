...
另有 94 道带答案的[章节代码练习与音频实验](../codes/research/05_exercises_and_audio.md)，从第 1 章到附录 B 使用 E01-01 等稳定编号。第 6 章 E06-07～E06-20 包含算法边界手算与子带、IPNLMS、RLS、Kalman 的小规模复算。每道题对应输入、执行代码和可核查结果，不替代原有练习题号。
...
三组反例见附录 B 图 35，非线性残留见第 6 章图 36。图 37 解释两带子带模型的交叉项；图 38 分别展示 IPNLMS、RLS 和 Kalman 的受控状态变化，三幅子图不构成性能排名。图 39 展示 AEC 方法的选择流程。

对应的 AEC 合成音频在研究手册第 15～16 节；谱减试听与前奏数字在第 18 节。这些音频与图 39 的流程图分别用于听辨和选型。

...
本页是全套教程的入口，教程正文一共 14 个文件：本页导读＋11 章正文＋2 篇附录。另有 5 篇[源码研究手册](../codes/research/README.md)网页，展开实现与工业复现，不计入正文篇数。
...
- 扫描法包括 Bartlett 与 Capon（最小方差谱估计）。例子里二者的峰旁比分别为 1.8 和 5.0。
...
再运行第 13.7 节列出的绘图命令，可重新生成 33 张图；运行时间取决于机器。具体构建步骤和错误处理见 `scripts/README.md`。Windows 上需把命令中的 `.venv/bin/python` 换成 `.venv\Scripts\python`。
...
先按第 8 节运行章节教学基线，确认公式约定和边界输入，再重绘 33 张图。之后可按锁定版本用 pyroomacoustics 建立房间仿真基线。公开数据、模型和硬件实验应根据任务、许可证与设备条件选择，练习入口见附录 B 的 §13.1、§13.6。

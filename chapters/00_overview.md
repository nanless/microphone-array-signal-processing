...
全套教程配了 35 张图，图 18～20、图 26～32 在第 6 章。插图分为几何示意、指定条件下的仿真和系统框图，阅读时要同时核对图注中的参数。
...
查阅加动手。13.1 节是学习路径。13.2 节是教材、会议、期刊、挑战赛与历史地图。13.3 节介绍八个研究方向与泛化失配。13.4 节是排错指南，13.5 节是公式速查卡，13.6 节有 17 道练习题及共同增益代码题，13.7 节说明怎样复现 35 张图。
...
6. 终点：重新生成并解释 35 张图，再在可用的多通道硬件上实现一个可测方向响应的基线系统。
...
再运行第 13.7 节列出的绘图命令，可重新生成 35 张图；运行时间取决于机器。具体构建步骤和错误处理见 `scripts/README.md`。Windows 上需把命令中的 `.venv/bin/python` 换成 `.venv\Scripts\python`。

另有 61 道带答案的[章节代码练习与音频实验](../codes/research/05_exercises_and_audio.md)，从第 1 章到附录 B 使用 E01-01 等稳定编号。每道题对应输入、执行代码和可核查结果，不替代原有练习题号。

36 个 WAV 样本由本书合成，分为 9 组，覆盖对齐平均、AEC、WPE、已知矩阵解混、工程失真、双声道声像，以及相关噪声、极性错误和病态求逆。先运行 `codes/examples/generate_audio_samples.py`，再按手册比较参考、输入和输出；网页提供不自动播放的试听控件。三组反例另见附录 B 图 35。
...
先按第 8 节运行章节教学基线，确认公式约定和边界输入，再重绘 35 张图。之后可按锁定版本用 pyroomacoustics 建立房间仿真基线。公开数据、模型和硬件实验应根据任务、许可证与设备条件选择，练习入口见附录 B 的 §13.1、§13.6。
...
- [ ] 对照第 8 节画一遍 35 张图，打开 `figures/` 对照正文看图。

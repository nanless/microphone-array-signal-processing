...
下面两题可用 [`exercises_spatial.py`](../codes/examples/exercises_spatial.py) 复算。程序输出按 E01-01、E01-02 标识，不需要下载数据。这里的数字是本书推导，不是设备实测。
...
相关系数为 1 时，四路噪声也同相相加，平均后原样保留。这个等相关模型用于隔离噪声相关性的影响；真实弥散场的相关性还随频率和麦间距变化，不能用一个固定 $\rho$ 描述整段语音。

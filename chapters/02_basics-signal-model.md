...
图 4（b）表明，纯时延对应单位模复数，只改变相位而不改变幅度。图 4（c）给出相位与频率的线性关系，其斜率为 $-2\pi\tau$；§4.2 的 GCC-PHAT 正是利用这项关系估计时延。
...
图 5（a）把房间脉冲响应分为直达声、早期反射和晚期混响；图 5（b）用能量衰减 60 dB 所需时间定义 $T_{60}$；图 5（c）展示给定房间参数下临界距离与体积、混响时间的关系。图中的距离只适用于相应参数，不能直接作为所有房间的分界。
...
图 5（c）是设定 $d_c=1.0$ m 的归一化示例。在该模型中，3 m 处的 DRR 为 $-20\log_{10}(3/1)\approx-9.5$ dB，表示混响能量高于直达声；这个数不能外推到未测量的房间。实际 $d_c$ 还随体积、吸声量和声源指向性变化。DRR 可作为距离估计的声学线索，但也会受房间和声源级别影响。[Montana State 房间声学讲义](https://www.montana.edu/rmaher/eele217_sp12/reverberation_time.pdf "citation")
...
它是 $M\times M$ 的复数矩阵，第 $m,n$ 个元素记录“麦 $m$ 与麦 $n$ 在该频点上的相关程度与相位差”（图6(c)）。它浓缩了阵列数据的二阶统计信息。Capon、MVDR、MUSIC 都以它为输入；MUSIC 再用特征分解把观测空间分成信号子空间和噪声子空间。只有在窄带远场模型正确、$K<M$ 个声源互不相干、噪声为空间白噪声且快拍足够多时，才可把高于噪声底的 $K$ 个特征值解释成 $K$ 个声源。相干反射、彩色噪声、有限快拍和阵列失配都会破坏这种整齐分组。

![图6 从波形到协方差矩阵](../figures/fig06_stft_cov.png)

图 6（a）（b）展示语谱图中的复数时频点，图 6（c）展示通道间相关性和相位差形成的协方差矩阵。图 6（d）只在独立声源、白噪声和充足快拍等条件下，才能把较大特征值的个数与声源数联系起来。

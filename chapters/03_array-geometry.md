...
**两种基础双麦摆法**（端射与正横是两个典型极限工作方向，中间角度同样有意义）：
...
| 形态 | 覆盖 | 优点 | 缺点 | 典型应用 |
|---|---|---|---|---|
| **均匀线阵（ULA）** | 一维 ±90° | 可直接利用平移不变结构；阵元集中在一条轴上，给定阵元数和间距时轴向孔径较大 | 不能由时延单独区分俯仰；靠近端射时 $\mathrm d\tau/\mathrm d\theta$ 趋近 0，角度误差被放大；还存在锥面模糊 | Soundbar、会议条 |

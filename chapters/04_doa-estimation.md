...
**图示说明**：（a）两路波形的设定真值为 $d=4$ cm、$\theta=30°$，对应 $\tau_0\approx58.3\,\mu$s，即 16 kHz 下约 0.933 个采样点；（b）将两路固定噪声样例做 16 倍时延网格插值后计算 GCC-PHAT，峰值落在 $58.59375\,\mu$s，即 0.9375 个原始采样点；虚线标出 $58.3\,\mu$s 真值。（c）时差乘声速得路程差，再除以间距得角度正弦（$\theta$ 从正横起算，与 §2.1 约定一致）。本次固定样例的估计接近真值，但单个样例不能代表误差分布。
...
第二行等于第一行乘 $q(f)$，因此行列式为零、秩为 1。旧写法把非对角元直接写成 0.5，只适用于 $q$ 恰为实数的特殊频点；上式保留了 3 个采样点延迟造成的复相位。于是 $\mathbf{A}\mathbf{S}\mathbf{A}^H$ 随之降为秩 1，本该两个的大特征值只剩一个。补救方法是**空间平滑**：ULA（uniform linear array，均匀线阵）切成 $L$ 个重叠子阵（$L\ge K$，即子阵个数不少于相干源数）、分别估协方差再平均，恢复可用秩。定量代价是有效子阵长度为 $M-L+1$，即用有效孔径换秩，分辨率也会降低；
...
**TOPS：避免依赖聚焦初值**。TOPS（test of orthogonality of projected subspaces，投影子空间正交性检验）不构造聚焦矩阵，而是把候选方向的参考导向矢量投影到各频点的信号或噪声子空间，检查正交关系在频率间是否一致。它不需要 CSSM 那样的聚焦初始角，但也不利用同一种跨频率相干合并；低 SNR 下的相对表现取决于频点选择、快拍数和实现。

**WAVES：按频点可靠度加权**。WAVES（weighted average of signal subspaces，加权信号子空间平均）按频点信噪比或子空间质量对信号子空间加权：可靠频点权重较大，噪声占优频点权重较小。它不要求精确的聚焦初始角，但结果依赖可靠度估计和权重设计，不能脱离实验条件给 CSSM、TOPS 与 WAVES 作统一排序。
...
两条代表路线如下：①FRIDA（finite rate of innovation DOA，有限新息率波到达方向估计）从连续参数模型估计指数和参数，再反解方向，不需要离散角度网格；其可辨识条件和误差受阵列几何、源数、标定与噪声影响。②原子范数（atomic norm）把离散 L1 正则推广到连续导向矢量集合，部分阵列模型下可转化为半定规划。两类方法避免了角度网格自身的量化误差，但通常比固定网格方法计算量更高。实时性、精度和源数条件必须绑定到具体实现与数据，不能用统一的 1° 门限选择算法。
...
| 特征 + 级联回归（IPDnet 一族） | IPDnet 类：CNN/CRNN（convolutional recurrent neural network，卷积循环网络）骨干 × 分类粗定 + 回归细化两级输出 | 输入相位差（IPD，inter-microphone phase difference，麦间相位差）与幅度谱特征，先做网格粗定位再回归细化；由网络联合宽带频点，不显式构造 CSSM 聚焦矩阵 | 可联合建模宽带与多源线索；跨房间和跨阵列泛化取决于训练覆盖与几何表示 |
| 空间谱学习 | Chakrabarty & Habets 2017/2019：CNN 骨干 × 空间谱/DOA 直接回归输出 | 声像图 → CNN 回归空间谱或 DOA；代表方法见 [arXiv:1807.11722](https://arxiv.org/abs/1807.11722 "citation") | 可输出多个方向；性能数字只适用于原论文的数据、阵列和噪声条件 |
...
| 序列模型 | CRNN/Transformer 骨干 × ACCDOA/multi-ACCDOA 输出格式（ACCDOA = activity-coupled Cartesian DOA，活动耦合笛卡尔坐标方向，Shimada et al., ICASSP 2021：向量长度表示活动强度，方向表示位置；例如 30° 方向的活动值为 0.9 时输出 $0.9\times(\cos30°,\sin30°)=(0.78,0.45)$，静音段输出接近零向量） | 利用时序上下文，联合输出声源活动与方向 | 可联合处理检测和定位；多源能力及混响鲁棒性取决于输出格式、训练数据和评测条件 |

**数据驱动结果的比较范围**：定位网络必须在相同数据集、阵列、信噪比、混响、训练数据和误差指标下比较。上表只归纳输入与输出形式，不给出跨论文排名。卷积空间谱定位的代表性方法可参见 Chakrabarty & Habets [arXiv:1807.11722](https://arxiv.org/abs/1807.11722 "citation")，期刊版 DOI [10.1109/JSTSP.2019.2901664](https://doi.org/10.1109/JSTSP.2019.2901664)。使用时还需评估标注成本、跨房间泛化和模型复杂度。
...
1. ★基础（TDOA + GDOP）：4 cm 双麦、正横方向附近，时延估计偏了 10 μs，角度会偏多少？（参考答案：正横附近 $\Delta\theta\approx c\Delta\tau/d=343\times10\times10^{-6}/0.04\approx0.086$ rad≈**4.9°**。线性近似只能在偏差足够小时使用。例如真值为 85° 时，时延约 116.2 μs；估计值增加 10 μs 会超过 4 cm 双麦的物理上限 $d/c\approx116.6$ μs，应判为越界或限制到端射。若减少 10 μs，精确反算为 $\arcsin[(116.2-10)\times10^{-6}\times343/0.04]\approx65.6°$，误差约 19.4°。这与 §3.1 算例 3-2 一致：端射附近的有限偏差必须用反正弦精确反算，不能把微分式的 56.4° 当成精确答案。）

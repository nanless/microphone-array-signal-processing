...
图 11 在同一个两源仿真（−20°、30°）中比较 Bartlett、Capon 和 MUSIC。条件为 8 元半波长 ULA、每阵元总输入 SNR 20 dB、400 个快拍，两源相隔 50°；这个较容易的条件只展示三种谱形，不检验最小可分辨角度。Bartlett 与 Capon 按各自的标准空间谱公式计算；MUSIC 给出伪谱，峰高不是功率。只有在源间隔、信噪比、快拍数、源数和阵列失配都明确时，才能用重复试验比较分辨成功率。
...
**Bartlett**：$P_B(\theta) = \vec{a}^H(\theta)\hat{\mathbf{R}}\,\vec{a}(\theta)$。$\hat{\mathbf{R}}=\frac{1}{T}\sum_t\vec{x}\vec{x}^H$ 的对角元是各通道自功率，非对角元是通道间互功率；二次型给出按候选导向矢量转向后的平均输出功率。若各方向的 $\|\vec a(\theta)\|$ 相同，省略统一归一化不改变峰位。Bartlett 的典型分辨尺度由该阵列的常规波束主瓣决定；Rayleigh 类判据是约定的分辨标准，不是所有噪声和检测条件下的绝对物理边界。
...
**SELD（声事件定位与检测，sound event localization and detection）**同时估计事件类别、方向和活动时段。STARSS23 是 DCASE 2023 Task 3 使用的带距离信息数据集示例；任务定义、划分和指标应按[当届官方任务页](https://dcase.community/challenge2023/task-sound-event-localization-and-detection-evaluated-in-real-spatial-sound-scenes)核对，不能把某一届设置外推为长期不变的“前沿”。
...
相当于约 $11.3\ \mu$s；在 4 kHz（$\lambda\approx85.8$ mm）处对应约 $16.2°$ 的均方根相位误差。球面角距网格可直接比较候选位置的相对路程差：候选 $(r,\theta)=(0.2\text{ m},20°)$ 与真值的均方根 TDOA 偏差约 18.6 μs，$(0.5\text{ m},30°)$ 约 6.8 μs，而真候选 $(0.2\text{ m},30°)$ 为 0。这个算例说明，角度相同不等于近场导向矢量相同；但能否可靠分出 20 cm 与 50 cm 仍取决于带宽、噪声、标定和网格分辨率。

...
本节先在单个频点分别讲 MUSIC 的正交检验和 ESPRIT 的子阵移位关系，二者都需要源数、有效协方差和相应阵列条件。随后 §4.6.1 才讨论宽带语音怎样合并多个频点；宽带聚焦不能代替前两种窄带方法各自的输入检查。
...
先看离散网格、连续参数和学习式定位分别改变了哪一步。随后 §4.7.1 检查这些方法也可能遇到的近场模型失配，§4.7.2 说明本书代码与外部实现的输入和验证边界。

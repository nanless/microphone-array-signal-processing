...
按 Benesty 与 Gay 的常用参数化，混合系数 $\alpha\in[-1,1]$：$\alpha=-1$ 退化为 NLMS，$\alpha=1$ 对应纯比例更新，$\alpha=0$ 对两部分等权混合。不同文献可能改写参数，比较数值前要先核对定义。声学回声路径通常不如网络回声路径稀疏，因此 IPNLMS 的收益需在实际路径上验证。
...
双讲时观测噪声估计增大，卡尔曼增益随之减小；路径变化使预测误差增大时，增益会上升以加快重新收敛。每个频点还需维护误差方差或协方差，因此计算量高于普通 PBFDAF。

...
图19 看什么：（a） 线性路径约 37 dB、非线性 tanh 路径约 19~20 dB，看的是两条线的差距、不是绝对值（实为与图31（a）同数据）；（b）算法时间线三锚点：1960 LMS / 1967 NLMS（随机梯度起点）、2006 FDKF（增益即最优时变步长）、2021 AEC Challenge（混合式登顶）——看分工是怎么一步步变细的。
...
> | 图19 | （a）线性约 37 dB vs 非线性约 20 dB 平台；（b）算法时间线 | §6.1.4、§6.1.6 | 看两条线的差距不看绝对值；（a）与图31（a）**同数据** |
...
> 📄 本篇信息：约 611 行 ｜ 字符约 47623 ｜ 配图 10 张 ｜ [回首页](./00_overview.md)

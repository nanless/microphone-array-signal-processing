...
三者残差依次为 10、20、30。图24和本章公式对应第一种离线索引，不能把逐帧接口的 `delay=1`直接改写成书中的 $\Delta=1$。
...
默认调用仍只返回谱数组。设 `return_diagnostics=True` 时另返回逐频点诊断：加载后正规矩阵在各轮中的最小数值秩、最大条件数、进入最小范数最小二乘回退的次数，以及未求解时的旁路原因。数值秩依赖浮点精度和矩阵尺度，不是语音质量或混响强度评分。当前基线在 `np.linalg.solve` 抛出奇异矩阵错误后用 `np.linalg.lstsq` 求最小范数解；它不会自行增加加载、减小 $K$ 或根据音质旁路。调用方应检查诊断和参考信号，在需要时调整加载、阶数或旁路策略；求得有限残差不等于保住了直达声。

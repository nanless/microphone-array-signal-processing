...
AEC 处理已知播放参考产生的回声，WPE 则利用语音跨帧预测结构估计晚期混响。它保留直达声和早期反射，只减去延迟若干帧之后的可预测成分。
...
$R$ 是 $K\times K$ 共轭对称矩阵，$K$ 典型取 5～20 时可用 Cholesky 分解求解。算法交替固定 $\lambda$ 求 $G$、固定 $G$ 更新 $\lambda$；第一次迭代的 $\lambda$ 常用整段观测能量初始化。在线版通常减少交替次数，并用递推统计量补偿样本不足。
...
**与其他模块的组合**：WPE 先减弱晚期混响，掩码 MVDR 再抑制噪声与定向干扰。Drude et al.（Interspeech 2018）在其测试条件下报告了联合系统相对单模块的改善。WPE → GSS → MVDR/GEV 也可不使用神经分离网络，见第 8 章。Raj et al.（Interspeech 2023）的 GPU 实现报告约 300 倍加速；引用时应注明其硬件、数据和基线。开源实现 `nara_wpe` 可直接调用，在线形式见下段。[Interspeech 2018 论文](https://www.isca-archive.org/interspeech_2018/drude18_interspeech.html "citation")、 [在线 WPE](https://www.isca-archive.org/interspeech_2017/kinoshita17_interspeech.html "citation")
...
#### 7.1.1 $\Delta$ 与 $K$ 的参数起点

以下换算采用 16 kHz、32 ms 窗和 8 ms 帧移。保护时间用于保留直达声和早期反射，$\Delta$ 约等于保护时间除以帧移；预测窗口覆盖需要建模的晚期拖尾，$K$ 约等于覆盖时间除以帧移。帧移改变时，应按物理时间同比调整 $\Delta$ 和 $K$。增加麦克风数可能减少达到同等估计稳定性所需的 $K$，但仍需在目标阵列上验证。
...
| 礼堂/教堂，T60 超过 1 s | $\Delta=4$，$K=15$~20，迭代 3~5 加 DNN 先验 | $\Delta=2$~3，$K=8$~10 加 DNN 先验 | 纯 WPE 回报递减，需结合目标数据验证 |
| 分布式大间距阵列 | 在上表基础上用麦克风相关延迟（Lohmann et al., ICASSP 2023：按到达时延补偿后定各通道延迟） | 同左 | 大间距下统一延迟会失真 |

$\Delta$ 应覆盖需要保留的早期反射，$K$ 应覆盖主要晚期拖尾。增大 $K$ 会增加计算量和过拟合风险。算力不足时可先减少交替迭代次数，再缩短 $K$；改变 $\Delta$ 前应先检查直达声和早期反射是否受到损伤。
...
> 📄 本篇信息：约 229 行 ｜ 字符约 13330 ｜ 配图 2 张 ｜ [回首页](./00_overview.md)

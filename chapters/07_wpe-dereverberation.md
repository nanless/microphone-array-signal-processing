...
**可执行 WPE 基线。** [`codes/array_tutorial/dereverberation.py`](../codes/array_tutorial/dereverberation.py) 提供只依赖 NumPy 的最小离线 WPE。`offline_wpe()` 接收复数 STFT，单通道轴序为 `(频点, 帧)`，多通道轴序为 `(频点, 通道, 帧)`，输出保持相同形状；它实现保护延迟、按滞后堆叠多通道历史、共享功率权重、相对对角加载和复共轭预测。

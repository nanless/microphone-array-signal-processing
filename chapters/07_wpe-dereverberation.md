...
**倒谱域加窗（liftering）**。先对频谱功率取对数，再做逆傅里叶变换，得到以“倒频率”为横轴的倒谱。声门激励是声带振动等产生的发声源信号；声道和房间又会改变它的频谱形状。这类单通道方法希望这些作用在倒谱域的不同区间较容易区分，于是用窗函数保留或压低指定区间。

计算量较低，但这些成分并不总能分开。重混响和强谐波会使它们在倒谱域重叠；窗口选错时，削弱房间成分也会同时损伤语音。
...
固定 $K=2$ 时，$\Delta$ 从 2 增至 3 并没有减少预测帧数量，仍使用两帧；它只是排除更多靠近当前帧的样本，并把长度相同的历史窗整体移远。$\Delta$ 的选择见 §7.8。

### 7.2 离线 WPE：交替估计预测器与语音功率
...
**算例 7-3：加权最小二乘求解器的手算（单抽头 $K=1$、3 个有效帧）**

取单频点、$K=1$、保护延迟 $\Delta=3$，只估计一个抽头 $g=G(3)$。从零开始计帧，完整六帧观测依次为 $X(0{:}5)=[0.8,\ 0.5,\ 0.4,\ 1.0,\ 0.6,\ 0.2]$。最早有效帧是 $n=3$；在 $n=3,4,5$，当前观测 $X(n)=[1.0,\ 0.6,\ 0.2]$，历史样本 $x(n)=X(n-3)=[0.8,\ 0.5,\ 0.4]$，权重 $w(n)=1/\lambda(n)=[1,\ 4,\ 1]$。前三帧只提供历史，不进入本例的三个正规方程求和项。
...
**共享功率还增加了一项模型选择。** 本书多通道实现用各通道预测残差的平方模均值更新一个共享 $\lambda(n,f)$，因此各输出通道使用相同的帧权重。这个平均来自把残差协方差限制为 $\lambda I_M$ 的球形复高斯工作模型；并非仅从“有 $M$ 个通道”就必然得到，也不保证真实早期声场在空间上独立同分布。若允许每通道不同方差或一般空间协方差，似然与更新都会改变。增益未校准的强通道会支配均值；[E07-06](#e07-06)给出推导、数字例子和边界。
...
这里必须把“先输出、后更新”的顺序和“`delay` 实际引用哪一帧”分开。固定四帧值 $[10,20,30,40]$，当前值为 40，零起始帧号 $t=3$；令 `taps=1`、`delay=1`，并人为固定预测抽头为 1。锁定的 0.0.11 版源码在这三种接口中采用不同的缓冲取点：

| 接口 | 实际用于预测的旧帧 | 当前值减去预测值 |
|---|---:|---:|
| 离线 `build_y_tilde()` | $X_{t-1}=30$ | $40-30=10$ |
| 无状态 `online_wpe_step()` | $X_{t-2}=20$ | $40-20=20$ |
| 有状态 `OnlineWPE.step_frame()` | $X_{t-3}=10$ | $40-10=30$ |

无状态接口传入以当前帧结尾的缓冲；有状态接口在写入当前值 40 **之前**先从自己的旧缓冲取历史。图24和本章公式对应表中第一行的离线索引，不能把逐帧接口的 `delay=1` 直接改写成书中的 $\Delta=1$。移植参数时，应先用这种带时间标签的小输入核对实际帧号。
...
**可预测目标的反例**：取一个频点、单通道六帧全为 $1+0j$，设 $K=2$、$\Delta=1$、迭代一次、对角加载为零。有效帧 $n=2,3,4,5$ 的历史向量全为 $[1,1]^\top$，所以 $\mathbf R=4\begin{bmatrix}1&1\\1&1\end{bmatrix}$、$\vec r=[4,4]^\top$；$\mathbf R$ 的秩为 1。最小范数解 $\vec g=[1/2,1/2]^\top$ 把这些帧预测为 1，残差为零（双精度实现中约在 $10^{-16}$ 量级）。前两帧因缺少完整历史而原样保留。
...
| nara_wpe 逐帧 | 同文件 `OnlineWPE` | 保留递推状态，每次处理一帧；0.0.11 的 `delay` 时间戳与离线接口不同 | 固定抽头手算实际历史帧；同一帧流按不同外层批次输入，检查是否误重置 |
| nara_wpe 块/递推 TensorFlow | `nara_wpe/tf_wpe.py` | 显式块统计与递推接口 | 对照帧时间戳，检查历史窗口和新输入是否混淆 |
| ESPnet DNN-WPE | `espnet2/enh/layers/dnn_wpe.py`、`wpe.py`、`mask_estimator.py` | 网络估计掩码/功率，解析层解预测滤波 | 先用固定正数功率核对求解，再加载训练好的模型参数；测未来帧依赖 |
...
本书另提供[独立实现数值对照](../codes/examples/compare_wpe_reference.py)：把教学版 `offline_wpe()` 与未经修改的 `nara-wpe 0.0.11` 的 `wpe_v6()` 比较。固定复数随机输入、预测阶数 2、保护延迟 3，检查单通道、双通道、通道置换和整体缩放，每种分别迭代 1 次和 3 次。两种实现对启动帧的处理不同，因此只比较零起始索引 $t=4$ 起的有效预测区间；本次 8 组检查的最大绝对误差约为 $3.2\times10^{-15}$。
...
[未来帧与多种外层分块实验](../codes/examples/wpe_temporal_contract.py)进一步固定种子 20260924，构造 48 帧、2 频点、1 通道的合成复谱；只在零起始第 30 帧的两个频点各加 $5+2\mathrm j$。锁定的 `nara-wpe 0.0.11` 逐帧接口保持同一个状态对象，使用 2 抽头、`delay=2`、遗忘因子 0.95。

两组输出在第 30 帧之前逐样本相同；第 30 帧的最大差为 $|5+2\mathrm j|=\sqrt{29}\approx5.385$，之后的最大差约为 1.261。当前帧计算时两次运行的旧状态和历史回归量相同，所以当帧输出差由输入改变量直接给出；后续差异还包含状态更新的影响。

将原始 48 帧分成 48 个单帧块、`7+13+5+23` 帧四块或 `30+18` 帧两块，调用同一对象的 `step_frame()`，与整段逐帧调用逐样本完全相同。这些“块”只是外层调用分组，**不是** `OnlineWPE.step_block()` 所要求的历史窗口。

作为相反对照，锁定版离线 `wpe_v6()` 用全段统计、一次迭代处理同一对输入时，第 30 帧以前的输出最大差约为 0.210。实验只验证固定复谱和固定实现的时间依赖；它没有 STFT 波形分帧、端到端延迟或去混响音质数据。[逐帧接口源码](https://github.com/fgnt/nara_wpe/blob/a166779cca2088817e330481bd20af1a2c598555/nara_wpe/wpe.py)与[Kinoshita 等 2017 年论文 §3](https://www.isca-archive.org/interspeech_2017/kinoshita17_interspeech.pdf "citation")分别给出此版本接口和在线短数据统计问题。
...
- **E07-05：WPD 的历史权重什么时候不为零？** 按式(7-6)、式(7-7)，取当前两麦及一个历史块组成四维向量，扩展导向为 $\bar v=[1,1,0,0]^\top$。给定加权协方差
    ...
    **解答**：解 $\bar R u=\bar v$。第 1、3 行给出 $2u_1+u_3=1$、$u_1+2u_3=0$，所以 $u_1=2/3,u_3=-1/3$；另两项为 $u_2=1,u_4=0$。分母 $\bar v^\top u=5/3$，得到 $\bar w=[0.4,0.6,-0.2,0]^\top$。约束为 $0.4+0.6=1$，目标值为 $\bar w^\top\bar R\bar w=0.6$。
    ...
    该矩阵特征值为 1、1、1、3，正定性和约束都可独立检查。这是本书给定协方差的优化手算，不是语音质量实验，也不将目标值下降当作普遍去混响收益。模型出处见 [Nakatani 与 Kinoshita，§II、§IV](https://arxiv.org/pdf/1908.02710)。

<a id="e07-06"></a>

**E07-06：共享 WPE 功率为什么除以通道数？** 固定一个频点、一帧和预测系数，不进行时间平滑。两通道复残差为 $E_1=1+\mathrm j$、$E_2=2$。在 $\vec E\sim\mathcal{CN}(0,\lambda I_M)$ 的工作模型下求共享方差，再把第二个通道乘以 2，比较估计。

**第 1 步：把通道数写入似然。** 对 $M$ 维 proper 复高斯向量，协方差行列式是 $\lambda^M$，指数中的二次型是 $\sum_m|E_m|^2/\lambda$。忽略与 $\lambda$ 无关的常数后，负对数似然及驻点为

$$\begin{aligned}
\ell(\lambda)&=M\log\lambda+\frac{\sum_{m=1}^{M}|E_m|^2}{\lambda},\\
\ell'(\lambda)&=\frac{M}{\lambda}-\frac{\sum_m|E_m|^2}{\lambda^2}=0,\\
\hat\lambda&=\frac1M\sum_{m=1}^{M}|E_m|^2\quad(\sum_m|E_m|^2>0).
\end{aligned}\tag{7-8}$$
...
生成式恢复是另一条路线。例如 [Stream.FM 预印本 v3（2026）](https://arxiv.org/abs/2512.19442v3)用逐帧流匹配处理包含去混响在内的语音恢复任务。它学习从受损观测到恢复信号的条件生成过程，不是用 WPE 的延迟线性回归替换一个更好的功率估计器。流式运行还要为数值求解的各阶段分别维护历史；网络单次调用因果不等于完整采样器已经能跨音频块续算。固定源码入口、已发现的状态接口问题与未执行边界见[研究手册 N14](../codes/research/02_aec_wpe_separation.md#streamfm)。本书没有运行该模型权重，不能据此声称它在本章合成例上优于 WPE。

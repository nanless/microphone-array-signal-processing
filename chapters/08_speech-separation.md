...
两麦 MVDR 先按每频点所有通道共同的实部/虚部峰值缩放观测，再计算统计量；解方程前还按干扰 SCM 的平均对角元素归一化。共同尺度会在 MVDR 的分子、分母中抵消，输出再恢复原尺度，因此不能用带绝对功率尺度的固定分母阈值决定旁路。目标或干扰掩码权重不足、零能量或求解失败仍退回参考麦；不把这些情形计为成功分离。

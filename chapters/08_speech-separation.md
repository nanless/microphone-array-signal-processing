...
| LibriCSS | 连续会议式重叠与长录音识别 | 扬声器在真实会议室回放 LibriSpeech 语音，包含不同重叠率 | 7 通道圆阵录音；每段约 10 min，输出需跨块保持一致 | 检查分块、跨块排列、输出流数和识别评测的说话人分配规则。见 [LibriCSS](https://arxiv.org/abs/2001.11482 "citation") |

...
EKF 用该观测函数的雅可比做一阶线性化。UKF 的另一种办法是：在当前估计的均值周围选出少量代表性状态点，把每个点分别代入原来的非线性观测函数，再由变换后的点重算均值和协方差。这些代表性点称为 sigma 点，并不是另外观测到的声源。

标准对称或缩放无迹变换的 UKF 通常传播 $2L+1$ 个 sigma 点；对位置—速度状态 $[x,y,\dot x,\dot y]^\top$，状态维数 $L=4$，因此是 9 个点。单纯方位—角速度状态只有 $L=2$，对应 5 个点。也有单纯形、球面径向等减少点数的变体，不能把 $2L+1$ 写成所有 UKF 的定义。
...
| IMM 交互多模型 | 按模式转移概率混合各模型初始状态，再分别预测、更新，以新息似然更新模式权重并融合 | 可在多个预设运动模式之间平滑切换 | 计算量随模型数增加；未包含的运动模式仍会失配，且需标定模型转移概率 | 包含启动、停止和匀速段的机动场景 |
...
**IMM 的模型切换**：交互多模型（Interacting Multiple Model，IMM）适合包含启动、停止和匀速段的轨迹。“交互”是每次预测前，各模型都按模式转移概率接收其他模型的状态信息；并非始终让几套滤波器互不联系，到最后才平均输出。

一次递推依次进行：按旧模式概率与转移矩阵求各新模式的先验概率；为每个新模式混合旧状态均值和协方差；从混合初值分别预测、更新；再用观测似然更新模式概率，融合最终估计。混合协方差须同时保留每个模型内部的误差，以及不同模型均值之间的差异。省略后一项会错误地认为“几个很确定、但互相矛盾的模型”合在一起仍很确定。[E09-09](#e09-09)给出分数手算。
...
本例假设预测后的十个粒子具有相同先验权重 $1/10$，例如上一步刚完成重采样。共同的先验因子与高斯密度常数会在归一化时消去，因此下面把未归一化权重直接写成高斯似然的指数部分：离观测越近分越高。若没有重采样且先验权重不同，必须保留上一帧的权重，完整递推和两次观测的反例见 E09-07。
...
PHD 的人数含义、递推和最小手算单独放在下一小节，便于与前面的逐帧数据关联方法对照。
...
|---|---|---|
| 坐标系标识（代码中可叫 `frame_id`） | 阵列坐标、设备坐标或世界坐标；角度零点与正方向 | 与导向矢量使用同一坐标约定；此处的 `frame_id` 不是音频帧序号 |
| 音频帧序号 | 这次定位所用音频块在录音中的次序 | 便于回查输入；不能代替下面的采样时间戳 |
...
- **E09-07：第二次观测为什么还要乘旧权重？** 两个静止粒子为 $0^\circ,10^\circ$，初始权重各 0.5。两次条件独立的观测恰好都为 $0^\circ$，标准差均为 $10^\circ$；中间不重采样，不加入杂波或过程噪声。计算两次后验，并比较每次直接覆盖权重的错误实现。

    **解答**：本题采用状态转移作为提议分布，即 bootstrap 粒子滤波。令 $g(z_t\mid x_t^{(i)})$ 表示第 $i$ 个粒子对本次观测的似然，则归一化递推为

    $$w_t^{(i)}=\frac{w_{t-1}^{(i)}g(z_t\mid x_t^{(i)})}{\sum_{j=1}^{N}w_{t-1}^{(j)}g(z_t\mid x_t^{(j)})}.\tag{9-13}$$

    两个似然的共同系数消去后为 $[1,e^{-10^2/(2\cdot10^2)}]=[1,e^{-0.5}]$。第一次权重是 $[1,e^{-0.5}]/(1+e^{-0.5})\approx[0.622459,0.377541]$。第二次再分别乘以同样的似然，得到 $[1,e^{-1}]/(1+e^{-1})\approx[0.731059,0.268941]$。错误的覆盖实现仍停在第一次结果，丢掉了旧观测的信息。

    这里是两次独立观测数值相同；把同一帧重复处理两遍会重复使用证据，也不正确。重采样之后先验权重重新均匀，但历史信息已经进入粒子位置和重复次数。若采用其他提议分布，还需保留状态转移与提议密度之比，此时式(9-13)的简化前提不成立。[Stone Soup 1.9.1 粒子滤波教程的权重递推](https://stonesoup.readthedocs.io/en/v1.9.1/auto_tutorials/04_ParticleFilter.html)给出一般形式，核实于 2026-09-26。

- **E09-08：发布时刻不是状态所属时刻。** 一个方向状态属于测量时刻 1.00 s，处理完成后于 1.05 s 发布，波束控制在 1.08 s 使用。状态为 $[30^\circ,30^\circ/\mathrm s]^\top$，协方差为 $P=\begin{bmatrix}4&1\\1&9\end{bmatrix}$，元素单位依次为度²、度²/秒、度²/秒²；有效期 0.10 s。采用 E09-04 的连续白角加速度模型，$q=2\ \mathrm{deg}^2/\mathrm s^3$，求使用时的状态和协方差。

    **解答**：外推间隔是 $1.08-1.00=0.08$ s，不能用 $1.08-1.05=0.03$ s。状态角度为 $30+30\times0.08=32.4^\circ$，直接使用旧角度会滞后 $2.4^\circ$。

    角度方差的传播先保留互协方差项：$4+2\times0.08\times1+0.08^2\times9=4.2176$。过程噪声再增加 $2\times0.08^3/3\approx0.000341333$；非对角元为 $1+0.08\times9+2\times0.08^2/2=1.7264$，速度方差为 $9+2\times0.08=9.16$。因此

    $$P_{1.08}\approx\begin{bmatrix}4.217941&1.7264\\1.7264&9.16\end{bmatrix}.$$
...
**E09-09：IMM 为什么不能只平均两个模型的方差？** 为隔离交互步骤，两模型都用同一局部展开角度作为标量状态，旧概率为 $\mu=[3/4,1/4]$，均值为 $x_1=0^\circ,x_2=10^\circ$，方差为 $P_1=1\ \mathrm{deg}^2,P_2=4\ \mathrm{deg}^2$。转移矩阵取 $T=\left[\begin{smallmatrix}0.9&0.1\\0.2&0.8\end{smallmatrix}\right]$，明确约定 $T_{ij}=\Pr(\text{新模式}=j\mid\text{旧模式}=i)$，所以**每行**和为 1。这里不是对真实停走过程的标定结果。

**第 1 步：先算到达新模式的概率。** 新模式 1 有两条来源，概率分别为 $(3/4)(9/10)=27/40$ 与 $(1/4)(1/5)=2/40$，合计 $c_1=29/40$。类似地 $c_2=3/40+8/40=11/40$。二者之和为 1。

**第 2 步：条件化，再混合状态。** 已知下一步选择新模式 $j$ 时，旧模式的条件概率是 $\omega_{i|j}=\mu_iT_{ij}/c_j$。对同维度、同坐标的欧氏状态，交互公式为

$$\begin{aligned}
c_j&=\sum_i\mu_iT_{ij},\qquad
\omega_{i|j}=\frac{\mu_iT_{ij}}{c_j},\\
x_j^0&=\sum_i\omega_{i|j}x_i,\\
P_j^0&=\sum_i\omega_{i|j}\left[P_i+(x_i-x_j^0)(x_i-x_j^0)^\top\right].
\end{aligned}\tag{9-14}$$

标量时最后一项就是均值差的平方；向量时必须是外积，不能逐元素平方后当作完整协方差。条件概率矩阵的每一**列**和为 1，与转移矩阵的行归一不同。

| 统计量 | 新模式 1 | 新模式 2 |
|---|---:|---:|
| 两个旧模式的条件权重 | $27/29,2/29$ | $3/11,8/11$ |
| 混合均值（度） | $20/29\approx0.689655$ | $80/11\approx7.272727$ |
| 模型内部方差项（度²） | $35/29\approx1.206897$ | $35/11\approx3.181818$ |
| 均值分歧项（度²） | $5400/841\approx6.420927$ | $2400/121\approx19.834711$ |
| 总方差（度²） | $6415/841\approx7.627824$ | $2785/121\approx23.016529$ |

新模式 1 的分歧项可以直接算为 $(27/29)(0-20/29)^2+(2/29)(10-20/29)^2$。另一种独立核算是“二阶矩减均值平方”：新模式 1 的 $P_1^0=(27/29)(1+0^2)+(2/29)(4+10^2)-(20/29)^2$。两个算法都给 $6415/841$。只保留 $35/29$ 会丢掉模型间的不确定性；混合方差比两项原方差都大，并不矛盾。

**第 3 步：把交互与观测更新分开。** 完整 IMM 接下来从各自的 $(x_j^0,P_j^0)$ 出发，使用对应运动模型预测并作观测更新。假设这两套更新额外给出的观测似然为 $L_1=0.2,L_2=0.8$，则未归一新权重为 $[29/200,44/200]$，归一后 $\mu^+=[29/73,44/73]\approx[0.397260,0.602740]$。这些似然是题设，用来复算贝叶斯权重；本题没有指定状态转移、过程噪声或观测值，不能冒称已经运行了两套 KF。最终状态融合也要保留更新后均值之间的外积项。
...
固定 [FilterPy `IMM.py`](https://github.com/rlabbe/filterpy/blob/3b51149ebcff0401ff1e10bf08ffca7b6bbc4a33/filterpy/kalman/IMM.py) 的 `predict()` 在传播前计算混合初值，`_compute_mixing_probabilities()` 实际执行 `mu @ M`；其类参数说明中的转移方向文字与这条赋值不一致，接入时应以明确约定和非对称矩阵小例核验，不能照抄文字后再转置一次。本书例题采用上述行转移约定，没有修改或运行该上游 IMM 类。

...
几何与时间口径固定如下：采样率 16 kHz，持续 2 s；两麦坐标分别为 $(-0.05,0)$ m、$(0.05,0)$ m。声源从 $(-0.8,1.5)$ m 出发，以 $(0.8,0)$ m/s 匀速运动，声速取 343 m/s。设发射时刻为 $u$，麦克风 $m$ 的接收时刻为 $t$，则本书求解
...
每 160 点保存一次轨迹真值。方位角从 $+y$ 轴向 $+x$ 轴为正，起点为约 $-28.07^\circ$，第 0.99 s 为约 $-0.31^\circ$，末段第 1.99 s 为约 $+27.83^\circ$。

把同一发射时刻到麦 1、麦 0 的传播时间之差定义为有符号 TDOA，起点为约 $+0.0001372$ s（$+2.19$ 采样），末段为约 $-0.0001361$ s（$-2.18$ 采样）。左侧声源更靠近麦 0，因而麦 1 的到达更晚；穿过正前方后符号反转。

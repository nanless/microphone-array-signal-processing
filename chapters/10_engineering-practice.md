...
图 25(b) 另行区分三条口径：连续增强音频只计其实际前瞻、缓冲、计算与调度；首个 KWS/ASR 判决还可能等待确认窗；**语音结束保持时间**是检测到能量下降后，仍等待一段时间才宣布语音结束，它只延后结束事件，不应自动计入连续音频或首个判决。例如额外保持 200 ms 会让结束事件晚 200 ms，不表示此前每一块增强音频都晚 200 ms。
...
以下十四题的输入均为本书构造的教学数据。先手算，再运行 E10-01～12 与 E10-14 的 [`exercises_engineering.py`](../codes/examples/exercises_engineering.py)：

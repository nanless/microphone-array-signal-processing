...
图 23 按较完整的语音交互系统绘制。远端音频先经过混音、均衡和音量控制；AEC 使用的**播放参考取点**位于这些处理之后、数模转换器（Digital-to-Analog Converter，DAC）和功率放大器之前。这样，参考信号包含实际播放时的混音与电平变化。

扬声器发出的声音经房间和机壳耦合回麦克风，形成需要消除的声学回声。图中虚线表示活动控制，实线表示音频、特征或统计量的依赖关系。
...
图 25 的关键路径条形把“采集/帧就绪等待”示意为 32 ms，再与未来帧前瞻 16 ms、首个判决确认 25 ms、计算 14 ms 和调度 6 ms 组成一条 93 ms 的示例关键路径。这里的 32 ms 是工作表示例，不等同于 STFT 窗长，也不是所有 32 ms 窗都会增加 32 ms 算法等待；窗的左右对齐、声卡周期、帧移和模块何时开始计算都会改变实际值。

图 25 的底部说明另行区分三条口径：连续增强音频只计其实际前瞻、缓冲、计算与调度；首个 KWS/ASR 判决还可能等待确认窗；**语音结束保持时间**是检测到能量下降后，仍等待一段时间才宣布语音结束，它只延后结束事件，不应自动计入连续音频或首个判决。例如额外保持 200 ms 会让结束事件晚 200 ms，不表示此前每一块增强音频都晚 200 ms。
...
声卡回调线程的首要目标是按时交付下一块音频。回调中临时分配大数组、等待互斥锁、访问磁盘、打印日志或执行时间不确定的模型推理，都可能让驱动错过期限。[PortAudio 官方回调说明](https://portaudio.com/docs/v19-doxydocs/writing_a_callback.html "citation")列出了分配、输入输出和互斥等可能产生无界等待的操作。
...
| AEC | 本机播放可能进入麦克风 | 麦克风；播放处理之后、数模转换器/功放之前的播放参考；远端活动/双讲状态 | 回声减弱的麦克风信号、残余回声与适配状态 | 参考是否覆盖混音、均衡和音量变化；参考—麦克风整体延迟、削波、双讲稳定性 |
...
| GSS 掩码估计 | 已知说话人活动且需要多人空间增强 | 分割活动标注；WPE 后的多通道 STFT | 各目标的时频软掩码、有效帧数 | 活动标注错位、掩码泄漏和目标缺席帧 |
| 目标/干扰 SCM 估计 | 已得到同一时频网格上的掩码 | 同一 WPE 阶段输出、未做方向归一化的多通道 STFT；对应目标/干扰掩码 | 每频点目标/干扰 SCM、有效样本数和条件数 | 掩码与 STFT 的时间、频点和通道约定须一致；掩码不是 SCM |
| 解析波束形成 | 需要将空间统计转换为目标音轨 | 同一阶段的多通道 STFT；目标/干扰 SCM 或导向矢量 | 每目标的增强信号及波束权重 | 导向失配、协方差病态和目标泄漏；方向状态只控制需要方向的波束 |
...
表中的 GSS 三行是一条有先后依赖的教学子链：活动标注约束掩码，掩码与多通道复系数形成 SCM，解析波束再用 SCM 或导向信息计算权重。若实现直接从网络预测 SCM，第二行的输入会改变；仍须记录实际取点，不能把三个中间量统称为“增强结果”。
...
**把时钟漂移变成可听、可查的样本。** 图 40 和主音频清单的 `clock_drift` 组使用标称 16 kHz、设备快 100 ppm、8 s 的确定性合成信号。两设备在各自的精确采样时刻对同一个连续信号求值，没有用左右声道电平差替代采样时刻差。以参考索引对应的 8 s 作换算，另一设备同索引时刻为 $8/(1+100\times10^{-6})$ s，差约 0.79992 ms，约为 12.799 个参考采样；实际文件末样本早于 8 s，逐样本真值以清单为准。
...
本书另用锁定的 libsoxr 0.1.3 做了**已知时钟真值**的受控变比率探针：48 kHz 名义输入前 5 s 快 100 ppm、后 5 s 快 150 ppm，输入共 480060 帧；输出 16 kHz，在输出帧 80000 切到新比率，用 160 个输出帧（10 ms）渐变。
...
$$
\Delta\phi=\frac{2\pi f}{c}\Delta\vec r^{\top}\vec u,
\qquad |\Delta\phi|\le\frac{2\pi f}{c}\|\Delta\vec r\| .
\tag{10-3}
$$
...
1. 确认通道编号和设备时间戳，在多个已知方向播放宽带信号。
2. 估计相对增益、固定时延和随频率变化的相位，同时核对实际麦克风坐标与坐标系。
3. 把系数连同设备序列、采样率、温度和标定日期保存。
4. 在未参与拟合的方向复测波束图和 DOA。
...
表中的**子空间法**先把多个频点或多个时刻的观测组织成向量，再从这些向量的协方差中寻找变化明显的方向。例如二维观测若有一个方向的方差远大于另一个方向，可以尝试把前者作为主要信号成分、削弱后者。这个判断需要信号与噪声在统计结构上可区分；两者落在相同方向，或观测太少以致协方差不稳定时，不能仅凭“保留较大特征值”分出语音。这里的二维例子只解释处理思路，不是语音子空间法的完整实现。

#### 10.3.2 功率谱减：一个频点怎样算到输出

先固定模型：单通道观测 $y[n]=s[n]+v[n]$，噪声在一小段时间内近似平稳；已经知道开头哪些帧只有噪声。对周期 Hann 窗、512 点短时傅里叶变换（STFT）和 128 点帧移，记频点 $f$、帧 $t$ 的复系数为 $Y_{f,t}$。本书在完整落入噪声段的帧集合 $\mathcal N$ 上，按同一窗和 FFT 归一化口径平均 $|Y|^2$，得到固定的逐频点噪声功率估计 $D_f$。这是**已知噪声段**的教学条件，不是自动检测无语音帧。

本书演示的功率谱减、相对观测功率的谱地板和复系数重建定义为

$$
\begin{aligned}
D_f&=\frac{1}{|\mathcal N|}\sum_{t\in\mathcal N}|Y_{f,t}|^2,\\
\widehat P_{f,t}&=\max\!\left(|Y_{f,t}|^2-\alpha D_f,\;\beta |Y_{f,t}|^2\right),\\
\widehat X_{f,t}&=\begin{cases}
\sqrt{\widehat P_{f,t}/|Y_{f,t}|^2}\,Y_{f,t},&|Y_{f,t}|^2>0,\\
0,&|Y_{f,t}|^2=0.
\end{cases}
\end{aligned}
\tag{10-4}
...
这里 $\alpha\ge0$ 是过减系数，$0\le\beta\le1$ 是**相对带噪频点功率**的地板比例；$\beta=0.04$ 意味着正功率频点的最小**幅度**增益为 $\sqrt{0.04}=0.2$。重建保留带噪相位；零系数没有相位，因此保持为零，不能凭谱地板捏造一个任意相位。逐帧逆变换后，按窗平方权重重叠相加。

Boll 的[原始谱减论文](https://doi.org/10.1109/TASSP.1979.1163209 "citation")讨论从无语音段估计噪声；Berouti、Schwartz 与 Makhoul 的[功率过减与谱地板论文](https://doi.org/10.1109/ICASSP.1979.1170788 "citation")讨论残余音乐噪声。本书选择 $\beta|Y|^2$ 作为容易复算的地板口径；引用论文的其他地板定义或数值不能直接当作本实现的参数。
...
能量 VAD 虽然简单，却能把控制逻辑讲清。启动阈值应高于继续阈值，避免能量在单一阈值附近来回切换；语音结束保持时间（hangover）让判决在能量下降后继续保持若干帧，减少词尾被截断。预卷缓存解决的是词首：VAD 刚触发时，把触发前保留的短时音频一并送给后端。三个时间量都要换算成帧数，并在短词、长停顿和脉冲噪声上分别验证。

[`HysteresisVAD`](../codes/array_tutorial/engineering.py) 的 `update(frame)` 输入是波形样本，内部计算均方根后与门限平方根比较；构造参数 `on_threshold`、`off_threshold` 则是线性均方能量，不接受 dB 阈值；把 dBFS 门限传给它会造成口径错误。`hangover_frames=2` 表示首次跌破继续阈值的两帧仍保持活动，第三帧才关闭。例如恒定幅度 0.3 的一帧，均方能量为 $0.3^2=0.09$，可跨过 0.08 的启动阈值；直接把标量 0.09 当作波形输入只得到 $0.09^2=0.0081$，不会启动。
...
训练后量化（Post-Training Quantization，PTQ）不一定需要同一种校准流程。以 ONNX Runtime 的激活量化为例，静态量化用代表性校准输入预先确定尺度与零点；动态量化在推理时根据当前激活计算这些参数，因此增加了运行时工作。量化感知训练（Quantization-Aware Training，QAT）则在训练阶段模拟量化误差。[官方量化文档的 Dynamic/Static Quantization 与 Method selection 小节](https://onnxruntime.ai/docs/performance/model-optimizations/quantization.html "citation")，核实于 2026-09-22。
...
[`q15_dot`](../codes/array_tutorial/engineering.py) 用 64 位整数累加乘积，只在输出时除以 $2^{15}$、按最近偶数舍入并饱和；目标 DSP 若使用不同累加位宽或逐乘积舍入，就不应期待 bit-exact 相同。测试既要覆盖正常小数，也要覆盖 $\pm1$、超范围输入和累加器极值；不能只比较平均误差。实数 PCM 接口还应在类型转换前拒绝意外的复数输入，不能把丢弃虚部后的数组当作原始信号。

**两抽头手算。** 两个输入向量均取 Q1.15 整数 $[16384,16384]$，每个码字表示 $0.5$。逐项整数乘积都是 $16384^2=268435456$，按 $2^{30}$ 缩放各表示实数 $0.25$。

64 位累加值为 $268435456+268435456=536870912$，表示实数 $0.5$。只在末尾除以 $2^{15}$ 得 $16384$，故 `q15_dot` 返回 Q1.15 码字 $16384$，反量化为 $0.5$。

**饱和边界。** 若两个向量都只含一个 $-32768$，乘积表示实数 $1$，换回 Q1.15 原本需码字 $32768$，超过正端上限，所以饱和为 $32767$。这些整数可由本书函数复核；采用窄累加器或逐项舍入的 DSP 必须另行比对。
...
式(10-5)约定 $P_{\text{avg}}$ 是各工作状态按占空比加权后、在稳压与转换电路**输出端**测得的整机负载平均功率；$\eta$ 才可同时计入电池可用容量比例、从电池到负载的转换效率及老化裕量。若功率仪测的是**电池端输入功率**，转换损耗已经包含在 $P_{\text{avg}}$ 中，$\eta$ 不应再次计入转换效率。应记录电压、测点、各状态功率与占空比；只给毫安时（mAh）而不给这些条件，无法推出可验收的续航。
...
表中的时间可逐行复算。记第 $i$ 帧的到达、开始、完成和服务时长为 $a_i,b_i,c_i,s_i$。一条不能抢占的串行工作线程满足 $b_i=\max(a_i,c_{i-1})$、$c_i=b_i+s_i$；从到达到完成的响应时长是 $c_i-a_i$。帧 3 在 30 ms 到达，但帧 2 要到 45 ms 才完成，所以帧 3 从 45 ms 处理到 49 ms，响应为 $49-30=19$ ms。帧 4 在 40 ms 到达，等帧 3 到 49 ms 结束后再处理 4 ms，响应为 $53-40=13$ ms。

计算量之和为 $45$ ms，观测的六帧音频长 $60$ ms，因而示意平均 RTF 为 $45/60=0.75$。在 $t=40$ ms 新帧到达后，帧 2 正在处理、帧 3 和帧 4 等待，恰有 3 帧在途。帧 3 的等待最长，为 $45-30=15$ ms。六帧虽然全部被接收，仍有帧 2、3、4 共 3 帧错过各自到达后 10 ms 的期限。
...
| 说话人分割 | 分割错误率（Diarization Error Rate，DER）、Jaccard 错误率（Jaccard Error Rate，JER） | 必须固定说话起止边界的容差时长（collar）、多人重叠片段怎样计分、参考区域和评分工具版本；分割正确不等于分离内容正确 |
...
[ITU-T P.835（07/2026）](https://www.itu.int/rec/T-REC-P.835/en "citation")用于对含噪语音处理系统做主观听测，分别评价语音信号（Signal，SIG）、背景噪声（Background，BAK）和总体效果（Overall，OVRL）。截至 2026-09-22，官方条目状态为“生效，预发布”（in force, prepublished）；三维评分的用途可见[官方工作项目说明](https://www.itu.int/itu-t/workprog/wp_item.aspx?isn=20844 "citation")。
...
|---|---|---|
| [WebRTC AEC3 固定提交](https://webrtc.googlesource.com/src/+/0467d2b91cc20b9b001c2bbb73d43ea6b2491f3e/modules/audio_processing/aec3/) | 实时 AEC 的延迟估计、线性消除和残余回声抑制分工，详见 §6.8 | 不能把内部常数当成本产品阈值，也不覆盖阵列定位和 WPE |
...
更多可直接阅读的工业实现包括以下几组。下表保留源码中的英文路径和接口名，方便查找；完整提交、许可和获取状态由 `codes/` 统一记录。
...
冷启动测量包含模型读取、图优化、权重上传和首轮内核准备；稳定运行延迟要在**预热**之后另测。预热是先用与正式测量相同的输入形状运行若干次，使首次初始化不混入稳定阶段的耗时；预热次数和是否计入报告都要说明。

更新模型前保存可回滚版本，并用同一硬件、同一数据和同一计时范围比较。
...
$$
\begin{aligned}
g_{k+1}&=g_k+\alpha_g(g_\star-g_k),\\
\alpha_g&=1-e^{-\Delta t/\tau_g},\qquad
g_k-g_\star=e^{-k\Delta t/\tau_g}(g_0-g_\star).
\end{aligned}
\tag{10-6}
$$
...
**E10-15：预卷、触发帧和结束保持怎样拼成一个片段？** 16 kHz 音频每帧 160 点，即 10 ms。启动/继续能量阈值为 0.08/0.03，结束保持为两帧，触发前缓存两帧。十帧从 0 编号，均方能量依次为 $[0,0.01,0.04,0.09,0.04,0,0,0,0,0]$。每帧用幅度为能量平方根的常数波形构造。求 VAD 活动帧、最终片段的采样区间、触发可用时刻和结束事件时刻。

**参考答案**：帧 2 的 0.04 只能达到继续阈值，不能启动。帧 3 的 0.09 启动，帧 4 保持活动；帧 5、6 消耗两次结束保持，帧 7 才关闭。活动标记为 `[假,假,假,真,真,真,真,假,假,假]`。

触发时先取出缓存中的帧 1、2，再追加触发帧 3 一次，之后保留帧 4、5、6。输出帧号为 `[1,2,3,4,5,6]`，对应半开采样区间 $[160,1120)$，长度 $1120-160=960$ 点，即 60 ms。若先把帧 3 入缓存又单独追加，会重复触发帧；若拿关闭的帧 7 作片段末帧，则额外保留了一帧。

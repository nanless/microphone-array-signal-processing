...
多通道采集 → AEC → WPE → 定位与追踪 → 波束形成 → 单通道 NS/AGC → KWS/ASR。
...
图 23 按较完整的语音交互系统绘制。虚线表示控制关系或可选旁路，实线表示音频或特征的传递关系。图中 AEC 的“最先处理”是指：启用 AEC 时，应在会破坏播放参考与回声之间线性关系的 NS、AGC 等处理之前消除回声，并不表示任何系统都必须以 AEC 开头。确定实际模块后，还要重新核对各信号取点。
...
若若干模块在同一帧内串行执行，它们的计算时间要相加；并行分支取耗时最长的关键分支；流水重叠的等待只沿关键路径计一次。在线 WPE 只使用历史帧时，本身不需要未来帧，但其分帧、平滑和实现缓冲仍可能产生延迟。
...
图 25(a) 把“采集/帧就绪等待”示意为 32 ms，再与未来帧前瞻 16 ms、判决确认 25 ms、计算与调度 20 ms 组成一条 93 ms 的示例关键路径。这里的 32 ms 是工作表示例，不等同于 STFT 窗长，也不是所有 32 ms 窗都会增加 32 ms 算法等待；窗的左右对齐、声卡周期、帧移和模块何时开始计算都会改变实际值。这些数字只说明记账方法，没有包含图外可能存在的解码和交互延迟。实际系统应在声学输入与最终输出之间记录时间戳或回采脉冲，分别测量冷启动、稳定运行和最不利调度条件。把 STFT 窗长从 32 ms 改为 16 ms，也不等于必然省下 16 ms：只有依赖完整窗或未来样本的等待发生变化，关键路径才会缩短。
...
| AEC | 本机播放可能进入麦克风 | 麦克风、播放参考 | 回声减弱的麦克风信号 | 参考取点、整体延迟、双讲稳定性 |
| WPE | 晚期混响影响任务 | 多通道或单通道 STFT | 去混响 STFT | 预测延迟、阶数、语音损伤 |
| 定位/追踪 | 波束或应用需要方向/轨迹 | 多通道信号 | 方向、置信度、轨迹 | 坐标系、时间戳、漏检与虚警 |
| 波束形成 | 需要空间选择性 | 多通道信号、方向或目标相对传递函数 | 增强信号 | 失配、白噪声增益、协方差估计 |

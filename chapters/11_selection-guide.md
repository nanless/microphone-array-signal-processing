...
同一设备示意电池为 3.7 V、2000 mAh，可用能量系数为 0.8；低功耗状态为 0.2 W、占时间 10%，处理状态为 1.2 W、占时间 90%。估计平均功率和续航。这里的输入是预算示意，不是某产品规格。

**答案要点**：评分覆盖为 $8/10=80\%$，3.8 只能标成成功子集的平均分；应保留总数 10、成功数 8、失败数 2、失败原因和各场景分布。缺失项不能擅自填 0 分，也不能悄悄从完整测试集删除；是否判整次评测无效应遵守预先规定的规则。

容量先换成 $2$ Ah，标称能量为 $3.7\times2=7.4$ Wh，可用能量为 $7.4\times0.8=5.92$ Wh。平均功率为 $0.2\times0.1+1.2\times0.9=1.10$ W，续航约 $5.92/1.10=5.38$ h。`E11-02` 输出这组中间值；温度、老化和状态转换消耗没有单独建模，所以它是估计，不是续航验收。

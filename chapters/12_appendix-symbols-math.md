...
| PBFDAF | 分区块频域自适应滤波：把长回声路径分块并用 FFT 计算卷积与更新。第 6 章式(6-3)与同配置的频域多抽头 NLMS 更新逐分量相同；相对逐样本时域实现能节省多少计算取决于滤波长度、分块、FFT 库和硬件（§6.2） |
| IPNLMS | 改进比例 NLMS：按抽头幅度分配各系数的更新量。常见定义中，混合系数 $\alpha=-1$ 退化为 NLMS，$\alpha=1$ 接近纯比例更新，$\alpha=0$ 是两部分的折中；具体取值要与所用公式和数据一同验证（§6.2） |

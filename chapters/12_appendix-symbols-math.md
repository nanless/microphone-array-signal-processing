...
本附录的 12.1～12.3 汇总符号、术语和数学工具，12.4 用小矩阵与短序列练习复算；下一篇是附录 B。
...
| WDO（W-不重叠正交性，W-disjoint orthogonality） | 近似认为不同源在时频平面的非零支撑较少重叠，从而可逐时频点选取主导声源；它强于“互不相关”。例如两组系数 $[1,1]$ 与 $[1,-1]$ 的内积为 0，但两个位置都同时非零，故不能由正交或零互协方差推出 WDO。强混响或多人同时发声会削弱这一近似（§8.1）。[Yılmaz 与 Rickard 原始论文](https://doi.org/10.1109/TSP.2004.828896 "citation") |
...
| PBFDAF | 分区块频域自适应滤波：把长回声路径分块并用 FFT 计算卷积与更新。第 6 章式(6-3)与同配置的频域多抽头 NLMS 更新逐分量相同；相对逐样本时域实现能节省多少计算取决于滤波长度、分块、FFT 库和硬件（§6.1.3） |
...
能量比用 $10\log_{10}(\cdot)$，幅度比用 $20\log_{10}(\cdot)$。系数相差一倍，是因为相同阻抗下功率与幅度平方成正比。
...
期望 $E[\cdot]$ 表示统计平均；方差描述变量偏离均值的平方平均；协方差描述两个中心化变量共同变化的程度。把 $M$ 个麦克风两两协方差排成矩阵，就是空间协方差矩阵。

§2.5 的常用估计式未显式减均值，因此在零均值假设下才与协方差相同；否则应称未中心化二阶矩。
...
TDOA 几何解算（§4.4）和 WPE 系数估计（§7.1）都会用到这一思想。
...
附录 B 第 8 题可比较加载前后的 WNG、DI 与条件数；数值由脚本参数决定，不预设固定区间。

### 12.4 计算练习

四题都可通过 `.venv/bin/python -m codes.examples.exercises_engineering` 复算。代码中的数组是本书构造的输入，答案先由短序列求和或代数得到，再与实现比较。

**E12-01：FFT 乘法为什么需要补零。** 取 $x=[1,2]$、$h=[1,0.5]$，下标都从 0 开始。求完整线性卷积，再比较 FFT 长度为 2 和 3 时的结果。
...
**E12-02：未中心化二阶矩为什么不总等于协方差。** 一个频点上有两路麦克风、两个快拍，列向量为 $\vec x_1=[1,\mathrm j]^\top$、$\vec x_2=[1,-\mathrm j]^\top$。分别计算以快拍数 2 为分母的二阶矩，以及先减样本均值的协方差。本题不使用无偏估计的 $T-1$ 分母。
...
第二行是第一行的两倍，只有 $x_1+x_2=2$ 这一条独立约束，所以所有解可写成 $[t,2-t]^\top$。范数平方为 $t^2+(2-t)^2=2(t-1)^2+2$，在 $t=1$ 取最小值；最小范数解为 $[1,1]^\top$，秩为 1。奇异值理论上为 $\sqrt{10}$ 和 0，浮点运算可能把后者算成接近零的数，因此还要记录判秩容差。

残差为零只说明输入方程被满足，没有说明两个参数可分别辨认。增加重复行不会改变这一点，需要新增独立观测或明确的先验约束。

**E12-04：频谱相位加权能使空间噪声白化吗？** 在一个固定频点，设两麦噪声零均值，协方差为 $\mathbf R_{nn}=\operatorname{diag}(4,1)$，两个通道的噪声功率单位相同。求一个对角白化矩阵 $\mathbf W$，算出白化后的噪声协方差；若目标导向矢量原为 $\vec a=[1,1]^\top$，处理后应使用哪一个导向矢量？最后判断只对互功率谱作 PHAT 加权能否得到同样的结果。

...
当 $\mathbf A$ 满列秩时，$\hat{\vec{x}}=(\mathbf{A}^H\mathbf{A})^{-1}\mathbf{A}^H\vec{b}$。秩不足或条件很差时，应使用 Moore–Penrose 伪逆、QR/SVD 或正则化，不能直接求逆。
...
**第 4 步：代回约束。** 用 $\vec w^H\vec a=1$ 定出比例系数，即得 §5.4 式(5-1)。LCMV 把标量约束和乘子换成向量形式（§5.5）。

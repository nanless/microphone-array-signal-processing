...
约定同 §2.3：**斜体 = 标量，头顶箭头 = 向量，加粗大写 = 矩阵**；$^H$ 共轭转置、$^\top$ 转置、$\hat{\ }$ 估计值；$X(f)$、$X(f_k,\ell)$ 类大写字母为逐频点复数**标量**（语音文献习惯）。本书采用前向傅里叶核 $e^{-\mathrm j2\pi ft}$。麦 1 位于 $x=0$，麦 2 位于 $+x$；正横为 0°，正角朝麦 2；$\vec u$ 从阵列指向声源，传播方向为 $-\vec u$。
...
|---|---|
| $M$, $d$, $D$ | 麦数、阵元间距、阵列孔径 |
| $c$, $\lambda$, $f$，$f_k$ | 声速（≈343 m/s）、波长、连续物理频率（Hz）、第 $k$ 个 DFT/STFT 频点对应的物理频率 $f_k=kf_s/N$ |

...
这里的 55 个 WAV 分成 13 组，由 `codes/examples/generate_audio_samples.py` 生成，使用 16 kHz、16 位 PCM。原有前六组使用种子 20260922，相关噪声、极性和病态求逆三组使用独立随机流 20260923；四麦分数采样组使用独立随机流 20260924，不改变此前的随机输入。非线性回声组没有随机输入。新增四种 AEC 方法组和两带跨项组的输入与固定种子见清单；其输出是指定合成模型的计算结果，不是真实录音或产品测量。

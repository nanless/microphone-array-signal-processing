...
对应 §4.2。本书 `doa.py::gcc_phat` 明确正时延约定、线性相关和三点插值；ODAS 的 `src/signal/xcorr.c`、`src/system/freq2xcorr.c` 可作为块处理参考。PHAT 消除互谱幅度权重，但低能量频点的相位会变得不可靠，因此归一化下限与有效频点筛选都是算法的一部分。

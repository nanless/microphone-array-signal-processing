...
与现有 Speex 记录的长度口径须分清：传给锁定版 `speex_echo_state_init()` 的请求长度是 4096，而 `mdf.c` 将其上取整为 $\lceil4096/160\rceil=26$ 个分区，即 4160 个系数位置。本教学实现设为 4160 是匹配**分区容量**，并不使两种更新律、输入处理或输出量化相同。两份 WAV 仅截到共同的完整 160 点块；单讲取 `[0,188320)`、双讲取 `[0,196160)`。单讲沿用 `[0,48000)` 适应与 `[48000,188320)` 评分；双讲沿用 `[0,48000)` 适应与 `[48000,128000)` 评分。

| 官方录音和评分区 | 参考条件 | PBFDAF 麦克风输入/float64 残差总数字功率变化 |
|---|---|---:|
| 远端单讲 `[48000,188320)` | 正确配对 | −6.700 dB |
| 同上 | 全零参考 | 0.000 dB |
| 同上 | 故意晚 1 s | −18.017 dB |
| 双讲 `[48000,128000)` | 正确配对 | −6.363 dB |
| 同上 | 全零参考 | 0.000 dB |
| 同上 | 故意晚 1 s | −1.665 dB |

指标对每一行都严格定义为 $10\log_{10}(\sum_{n\in W}d[n]^2/\sum_{n\in W}e[n]^2)$；负值表示本次浮点残差的**总功率更高**，不能改写成“负 ERLE”或回声分量变大多少。零参考下初始权重为零，输出按代码等于原麦克风，所以恰为 0 dB。故意晚参考的全文件残差分别有 1331、11 个样本超出 PCM16 可表示区间；上述数字按未削波的 float64 计算，**不能**与剪裁后的试听文件混用。脚本还记录每个评分子区间、原文件摘要、输出 float64 小端字节摘要和被截尾样本数；相邻子区间不是独立试验，整段值先汇合平方和再取对数。复做：

```bash
.venv/bin/python -m codes.examples.aec_pbfdaf_real_pair_compare --pair both
```
...
最小实验使用两条独立播放参考与两条已知 FIR；失败实验把两路参考设成完全相同。此时总回声可能抵消得很好，但单独的两条路径不一定可辨识。工程上应同时检查参考相关性、路径估计与重新混音后的恢复，而不是只报告合成回声误差。正文条件见 [§6.9](../../chapters/06_aec.md#sec-6-9)。
...
E08-04 对人为指定的解混矩阵演示回投影：原输出 `[2s, -3u]` 乘参考麦系数 `[1/2, -1/6]` 后成为 `[s, 0.5u]`。它恢复参考麦源图像而非统一干声尺度，且不把给定解混矩阵的代数操作冒充 AuxIVA 估计。E08-05 单独检查掩码 SCM 的分母地板；E08-06 则把第二块的两个输出槽位交换，展示“每块 PIT 都是 20 dB”仍不能保证直接拼接后的说话人身份连续。三题的输入、答案和边界见 [第 8 章练习](../../chapters/08_speech-separation.md#sec-8-8)。

...
```bash
.venv/bin/python codes/examples/generate_audio_samples.py  # 先生成 6 组、23 个合成 WAV 及清单
.venv/bin/python scripts/make_figures.py      # 生成图 1～25、图 33～34 → figures/
...
| 脚本 | 作用 | 输出 |
|---|---|---|
| `../codes/examples/generate_audio_samples.py` | 生成 6 组、23 个合成音频文件；清单记录参数、共同增益和摘要。`--check` 只检查现有生成物 | `codes/audio/*.wav`、`codes/audio/MANIFEST.json` |
| `make_figures.py` | 生成图 1～25 和图 33～34。只用 numpy 和 matplotlib，不依赖 scipy；随机种子固定。图 34 读取已生成的音频，必须先运行音频生成器。图 13 的蒙特卡洛统计耗时最长 | `figures/fig01`～`fig25_*.png`、`fig33_*`、`fig34_*` |
...
| `quality_check.py` | 发布门禁。用独立基线检查 14 篇/84 节/21 个指定子节/34 图，核对图号、alt、公式编号与引用、小节语义链接、PNG 绘图脚本摘要、网页导航和 PDF 三级书签。确定性问题阻断发布，高风险口语只提醒人工复核 | 通过、失败清单，以及不阻断发布的人工复核与可访问性提示 |

改图练习（如附录 B 习题）应使用脚本副本或独立输出目录，记录改变的参数，不覆盖本书的发布图。新增 36 道可运行题使用三个模块入口：
...
**独立结构基线**：发布门禁的独立结构基线为 14 个顶级书签、84 个二级书签、21 个三级书签，共 119 个大纲项，以及图 1～34。它还检查图号与 alt、公式编号与引用、小节语义链接、每个源 h2/h3/h4 标题是否真的出现在当前页导航中（源 h1 可排除），以及 PNG 中的 `SourceScript` 和完整 `SourceScriptDigest`。
...
**扩展依赖**：`pyroomacoustics==0.10.0`（房间声学仿真库）只用于第 10 章示例和附录 B 的房间仿真练习；运行 `.venv/bin/pip install pyroomacoustics==0.10.0` 安装。不装也能生成正文的 34 张图。固定版本说明见[官方 PyPI 页面](https://pypi.org/project/pyroomacoustics/0.10.0/)。
